...
export class FormulariosComponent implements OnInit,AfterViewInit {
  ...
  constructor(private fb: FormBuilder, private formularioService: FormularioService,
    ...
    private chatService: ChatService,private cdr: ChangeDetectorRef,private router: Router, private location:Location
  ) {
  ...
  private setupNavigationTracking() {
    ...
    this.routerSub = this.router.events
      .pipe(
        filter((eventual: RouterEvent): eventual is NavigationEnd => event instanceof NavigationEnd)
      )
      .subscribe((eventual: NavigationEnd) => {
        if (!this.shouldIgnoreRoute(eventual.urlAfterRedirects)) {
          if (this.navigationHistory[this.navigationHistory.length - 1] !== eventual.urlAfterRedirects) {
            this.navigationHistory.push(eventual.urlAfterRedirects);
            console.log('Historial actualizado:', this.navigationHistory);
          }
        }
      });
  }
  ...
  private shouldIgnoreRoute(url: string): boolean {
    const ignoreRoutes = ['/login', '/registro'];
  ...
  goBack() {
    if (this.navigationHistory.length > 1) {
      ...
      const previousUrl = this.navigationHistory.pop() || '/vistaClient';
      this.router.navigateByUrl(previousUrl);
    } else {
      this.location.back();
      setTimeout(() => {
        if (this.router.url === this.navigationHistory[0]) {
 ...
 isScrollButtonVisible: boolean = true;  // Visible por defecto
    ...
    onScroll(): void {
      ...
      // Determinar si estamos en la parte superior (con margen de 20px)
      this.isAtTop = scrollPosition < 500;
      
      // Siempre mostrar el botón, pero cambiar su función
      this.isScrollButtonVisible = true;
    }
    
    scrollToTarget(): void {
      if (this.isAtTop) {
        // Bajar al final si estamos arriba
        window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
      } else {
        // Subir si estamos abajo
